...
class Gr00tPolicy(BasePolicy):
    ...
    def _load_model(self, model_path):
        ...
        # Apply converted QuantVLA weights when configured. This path reconstructs
        # QuantVLA-transformed W4 tensors into GPTQ-like qweight/scales and then
        # executes either through vLLM GPTQ-Marlin (real) or torch dequant (fake).
        if os.environ.get("QUANTVLA_CONVERTED_CHECKPOINT"):
            try:
                from gr00t.quantization import enable_quantvla_converted_if_configured
                enable_quantvla_converted_if_configured(model)
            except Exception as e:
                print(f"[GR00T] Converted QuantVLA replacement failed: {e}")
                raise
        # Apply explicit GPTQ LLM replacement when configured. This is mutually
        # exclusive with DuQuant because both paths replace nn.Linear.
        #   GR00T_GPTQ_QUANT_MODE=real -> vLLM GPTQ-Marlin Linear
        #   GR00T_GPTQ_QUANT_MODE=fake -> same GPTQ tensors, torch F.linear
        elif os.environ.get("GR00T_GPTQ_CHECKPOINT") or os.environ.get("GR00T_GPTQ_MARLIN_CHECKPOINT"):
            try:
                from gr00t.quantization import enable_gptq_quant_if_configured
                enable_gptq_quant_if_configured(model)
            except Exception as e:
                print(f"[GR00T] GPTQ LLM replacement failed: {e}")
                raise
        else:
